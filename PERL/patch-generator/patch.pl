...
if($argc < 1){ 
    print "usage:\n";
    die "patch inputfile [outputdir]\n";
}
my $in = $ARGV[0];
my %db;
open FD_IN,"<$in" or die "can't open file $in $!";
while(<FD_IN>){
    if(/.+:\d+:.+/){
        (my $f) = split /:/;
        if(!exists $db{$f}){
...
foreach (keys %db){
    my $src = $_; 
    if($ARGV[1]){
        s#/home/clxk/AAA/firmwarePrj/firmware#$ARGV[1]patch#;
    }else{
        s#/home/clxk/AAA/firmwarePrj/firmware#patch#;
    }   
    my $dst = $_; 
    print "copying ...\n\t$src\nto\n\t$dst\n";
    fcopy($src,$dst) or die $!;
}
